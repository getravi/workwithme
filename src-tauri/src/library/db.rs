//! SQLite persistence for the capture library.
//!
//! The database lives at `<data_local_dir>/workwithme/captures/library.db`.
//! Every public function that mutates state has a `_conn` sibling that accepts
//! an explicit `&Connection`, used by tests (with an in-memory DB) and by
//! `#[cfg(test)]` helpers.
//!
//! # Schema
//! | Table | Purpose |
//! |---|---|
//! | `captures` | One row per image/video capture |
//! | `captures_fts` (FTS5) | Full-text search over `app_name`, `window_title`, OCR text |
//!
//! `media_type` and `thumbnail_path` were added in a later schema version and
//! are migrated in using `ALTER TABLE ADD COLUMN` with safe defaults so
//! existing databases are upgraded on first open.
...
pub fn save_draft(
    ...
    let conn = db()
        .lock()
        .expect("mutex poisoned: SQLite library connection");
    save_draft_conn(&conn, file_path, app_name, window_title, width, height)
}

pub fn save_draft_conn(
    conn: &Connection,
    file_path: &str,
    app_name: Option<&str>,
    window_title: Option<&str>,
...
pub fn finalize_conn(conn: &Connection, id: &str) -> Result<(), String> {
    conn.execute(
        "UPDATE captures SET is_draft = 0 WHERE id = ?1",
        params![id],
    )
...
pub fn list_conn(conn: &Connection, before_ts: Option<i64>) -> Result<Vec<CaptureEntry>, String> {
    ...
    let entries = stmt
        .query_map(params![ts_filter], row_to_entry)
        .map_err(|e| format!("query list: {e}"))?
        .filter_map(|r| {
            r.map_err(|e| eprintln!("[library/db] list row error: {e}"))
                .ok()
        })
        .collect();
    Ok(entries)
...
pub fn search_conn(conn: &Connection, query: &str) -> Result<Vec<CaptureEntry>, String> {
    // An empty phrase literal ("") is invalid FTS5 syntax and produces a SQLite error.
    if query.trim().is_empty() {
        return Ok(Vec::new());
    }
    // Wrap in double-quoted phrase to prevent FTS5 query injection (same pattern as voice_db.rs).
    // FTS5 MATCH accepts its own query language; phrase-quoting forces a literal string search.
    ...
    let entries = stmt
        .query_map(params![quoted], row_to_entry)
        .map_err(|e| format!("query search: {e}"))?
        .filter_map(|r| {
            r.map_err(|e| eprintln!("[library/db] search row error: {e}"))
                .ok()
        })
        .collect();
    Ok(entries)
...
pub fn delete(id: &str) -> Result<Option<String>, String> {
    ...
    let file_path: Option<String> = conn
        ...
        .ok();
    // Use conn.transaction() so that if either DELETE fails the transaction is automatically
    // rolled back on drop — no dangling open transaction on the connection.
    let tx = conn.transaction().map_err(|e| format!("begin: {e}"))?;
    tx.execute("DELETE FROM captures WHERE id = ?1", params![id])
        .map_err(|e| format!("delete: {e}"))?;
    tx.execute("DELETE FROM captures_fts WHERE id = ?1", params![id])
...
pub fn prune_conn(conn: &Connection) -> Result<Vec<String>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT file_path FROM captures WHERE timestamp < ?1")
        .map_err(|e| format!("prepare prune: {e}"))?;
    let paths: Vec<String> = stmt
        .query_map(params![cutoff], |r| r.get(0))
        .map_err(|e| format!("query prune: {e}"))?
        .filter_map(|r| {
            r.map_err(|e| eprintln!("[library/db] prune row error: {e}"))
                .ok()
        })
        .collect();
    ...
    conn.execute(
        "DELETE FROM captures_fts WHERE id NOT IN (SELECT id FROM captures)",
        [],
    )
...
mod tests {
    ...
    fn test_prune_removes_old_entries() {
        ...
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM captures WHERE id='old-id'", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(count, 0);
    ...
    fn test_search_empty_query_returns_empty_not_error() {
        let conn = open_in_memory().expect("in-memory DB");
        // An empty or whitespace query must not propagate a SQLite FTS5 syntax error.
        assert_eq!(search_conn(&conn, "").unwrap().len(), 0);
        assert_eq!(search_conn(&conn, "   ").unwrap().len(), 0);
    }
}
