...
import type { AuthStatus, OAuthProvider, PendingOAuthFlow } from "./types";

export function ConnectionsTab({ isConnected }: { isConnected: boolean }) {
  ...
  const handleOAuthLogin = (providerId: string) => {
    void (async () => {
      ...
      try {
        ...
        setErrorMessage((e instanceof Error ? e.message : String(e)) || "OAuth failed.");
        setOauthInstructions(null);
        setOauthProgress("");
        setPendingOAuthFlow(null);
        updateStatus("error");
  ...
  const handleOAuthComplete = () => {
    void (async () => {
      ...
      try {
        ...
        setErrorMessage((e instanceof Error ? e.message : String(e)) || "OAuth completion failed.");
        updateStatus("error");
      }
    })();
  };
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-start gap-2">
                  <button
                    ...
                    className="flex-1 text-left text-[12px] text-[#c5f016] hover:underline break-all select-text"
                  >
                    {oauthInstructions.url}
                  </button>
                  <button
